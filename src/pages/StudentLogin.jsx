...
export default function StudentLogin() {
  ...
  const handlePasswordLogin = async (e) => {
    e.preventDefault();
    if (!email || !password) return setError('Please enter your student email and password.');
    ...
    try {
      const data = await api.login(email, password, 'STUDENT');
      setLoading(false);
      if (data.user) {
      ...
      setLoading(false);
      setError(err.message || 'Student login failed. Please verify credentials.');
    }
  };
    ...
    try {
      ...
      setInfoMessage(
        data.otpPreview
          ...
          : data.message || `A verification OTP has been sent to ${email}`
      );
    } catch (err) {
      setLoading(false);
  ...
  const handleOtpLogin = async (e) => {
    e.preventDefault();
    if (!email || !otp) return setError('Please enter both email and the 6-digit OTP.');
    ...
    try {
      const data = await api.loginWithOtp(email, otp, 'STUDENT');
      setLoading(false);
      if (data.user) {
        localStorage.setItem('sporic_user', JSON.stringify(data.user));
      }
      ...
      <div className="container" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 'calc(100vh - var(--nav-height))', padding: '2rem 0' }}>
        <GlassCard glow className={styles.formCard} padding="xl">
          ...
          <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
            ...
            <h2 className={styles.title} style={{ marginTop: '0.75rem' }}>Student Login</h2>
            <p className={styles.subtitle}>Access your enrolled courses, certificates, and learning syllabus.</p>
          </div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem', background: '#FAFAFA', padding: '4px', borderRadius: '8px', border: '1px solid #E5E5E5' }}>
            <Link to="/login/student" style={{ flex: 1, textAlign: 'center', padding: '0.5rem', borderRadius: '6px', background: '#000000', color: '#FFFFFF', fontWeight: 700, fontSize: '0.8rem', textDecoration: 'none' }}>
              Student
            </Link>
            ...
            <form onSubmit={handlePasswordLogin} className={styles.form}>
              <div className={styles.inputGroup}>
                ...
                <input
                  type="email"
                  required
                  placeholder="student1@vit.ac.in"
                  value={email}
